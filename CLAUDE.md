...
python -m franka_control.scripts.collect_episodes --robot-ip <控制机IP> --repo-id user/dataset --root data/dataset --control-mode ee_delta --device spacemouse --fps 30
...
└── scripts/                 # teleop / collect_episodes / collect_waypoints / run_trajectory / measure_latency
```
...
**Data 模块已完成**：LeRobot v3.0 格式数据采集系统，支持 4 种控制模式（joint_abs/delta, ee_abs/delta）、多相机、阻塞操作期间状态流。单元测试通过（45/45），真机测试计划已就绪（`tests/REAL_ROBOT_TEST_PLAN.md`）。
