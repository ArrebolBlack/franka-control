...
7. **教用户** — 用户问就解释，帮用户理解
8. **修改前列计划** — 任何文件修改前，先整体列出：拟修改文件、修改内容、原因、风险、验证方式。用户确认后才动手，确保不损坏现有功能
...
测试覆盖：`test_gripper.py`（23 个测试用例）、`test_keyboard_teleop.py`（8 个测试用例）、`test_data_collection.py`（14 个测试用例），共 45 个测试。其余模块（env、robot、spacemouse_teleop、camera、trajectory）暂无测试。
...
python -m franka_control.scripts.collect_episodes \
    ...
    --device spacemouse \
    --fps 30 \
    --num-episodes 50  # 录制 50 个 episodes

# Resume 继续录制（追加更多 episodes）
python -m franka_control.scripts.collect_episodes \
    --robot-ip <控制机IP> \
    --repo-id user/dataset \
    --root data/dataset \
    --resume \
    --num-episodes 20  # 追加 20 个 episodes
...
├── data/                    # LeRobot v3.0 格式数据采集
│   ├── collector.py         # DataCollector 类（纯记录器）
│   ├── state_recorder.py    # StateStreamRecorder（后台线程读取状态+相机）
│   ├── config.py            # CollectionConfig + CameraConfig
│   └── features.py          # build_franka_features()
├── trajectory/              # Waypoint YAML + TOPPRA 轨迹规划
└── scripts/                 # teleop / collect_episodes / collect_waypoints / run_trajectory / measure_latency / play_dataset
```
...
- **多 episode 录制**：`--num-episodes N` 连续录制多个 episodes
- **Resume 追加**：`--resume` 从已有 dataset 继续录制，自动递增 episode index
- **Success 标注**：每个 episode 结束时标注 success/failure，保存到 `meta/episode_annotations.json`
- **后台状态流**：StateStreamRecorder 后台线程持续读取 robot state + 相机，夹爪阻塞期间数据不丢失
- **4 种控制模式**：joint_abs/delta, ee_abs/delta
- **多相机支持**：通过 CameraConfig 配置多个 RealSense 相机

### 数据集播放器功能

`scripts/play_dataset.py` 提供完整的数据集可视化和分析：
...
- Detailed 模式显示完整状态（joint pos/vel, ee pos/quat, gripper, action）
